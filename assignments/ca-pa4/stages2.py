...
class IF(Pipe):
    ...
    def compute(self):
        ...
        imm = RISCV.imm_j(self.inst) if (br_type == BR_J) else RISCV.imm_b(self.inst)
        predict_brjump_target = Pipe.cpu.adder_if.op(self.pc, imm)
        predict_taken = (br_type == BR_J) or \
                        ((br_type != BR_N) and (br_type != BR_JR) and imm < 0)
        # Select appropriate PC
        if Pipe.ID.c_misprediction:
            self.pc_next = Pipe.EX.pcplus4 if Pipe.ID.c_pc_sel == PC_MISPREDICT else \
                Pipe.EX.brjump_target if Pipe.ID.c_pc_sel == PC_BRJMP else \
                    Pipe.EX.jump_reg_target if Pipe.ID.c_pc_sel == PC_JALR else \
                        WORD(0)
        else:
            self.pc_next = predict_brjump_target if predict_taken else \
                self.pcplus4 if Pipe.ID.c_pc_sel == PC_4 else \
                    WORD(0)

        # TODO Use Pipe.cpu.ras for the return address stack
    ...
    def compute(self):
        ...
        prev_load_use_hazard = (EX_mem_read and (EX.reg_rd != 0)) and \
                               ((EX.reg_rd == self.rs1 and self.c_rs1_oen) or (
                                           EX.reg_rd == self.rs2 and self.c_rs2_oen))
        ...
        pprev_load_use_hazard = (M1_mem_read and (M1.reg_rd != 0)) and \
                                ((M1.reg_rd == self.rs1 and self.c_rs1_oen) or (
                                            M1.reg_rd == self.rs2 and self.c_rs1_oen))
        ...
        taken = pc_sel != PC_4
        predict_taken = Pipe.EX.pcplus4 != 0 and Pipe.EX.pcplus4 != self.pc
        self.c_misprediction = taken != predict_taken

        if not self.c_misprediction:
            self.c_pc_sel = PC_4
        elif not taken:
            self.c_pc_sel = PC_MISPREDICT
        else:
            self.c_pc_sel = pc_sel

        # On stall: IF, ID: stall, EX: bubble
        # On c_misprediction: ID, EX: bubble
        self.IF_stall = hazard
        self.ID_stall = hazard
        self.ID_bubble = self.c_misprediction
        self.EX_bubble = hazard or self.c_misprediction
