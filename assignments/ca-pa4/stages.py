...
class IF(Pipe):
    ...
    def compute(self):
        ...
        if opcode in [ EBREAK, ECALL ]:
            ...
            # self.inst = BUBBLE
            opcode = RISCV.opcode(BUBBLE)

        cs = csignals[opcode]
        br_type = cs[CS_BR_TYPE]
        imm = RISCV.imm_j(self.inst) if (br_type == BR_J) else \
              RISCV.imm_i(self.inst) if (br_type == BR_JR) else \
              RISCV.imm_b(self.inst)
        rd = RISCV.rd(self.inst)

        predict_jump_target = Pipe.cpu.adder_if.op(self.pc, imm)
        predict_taken = (br_type == BR_J) or \
                        ((br_type != BR_N) and (br_type != BR_JR) and SWORD(imm) < 0)
        predict_func_call = (br_type == BR_J or br_type == BR_JR) and rd == 1
        predict_func_return = br_type == BR_JR and rd == 0 and imm == 0

        # RAS handling
        pop_status = False
        if predict_func_return:
            ras_pop, pop_status = Pipe.cpu.ras.pop()
        if predict_func_call:
            Pipe.cpu.ras.push(self.pcplus4)

        self.pc_next =  Pipe.ID.correct_jump_target if Pipe.ID.c_pc_sel == PC_MISPREDICT else \
                        ras_pop if predict_func_return and pop_status else \
                        predict_jump_target if predict_taken else \
                        self.pcplus4 if Pipe.ID.c_pc_sel == PC_4 else \
                        WORD(0)
    ...
    def update(self):
        ...
        # Update IF
        # Update pc only when not if_stall
...
class ID(Pipe):
    ...
    def compute(self):
        ...
        # EX/M1/M2/WB signal: can be used
        ...
        # Control Signals --------------------------------------------
        # Hazard detection
        EX_mem_read = EX.reg_c_dmem_en and EX.reg_c_dmem_rw == M_XRD
        prev_load_use_hazard = (EX_mem_read and (EX.reg_rd != 0)) and \
                               ((EX.reg_rd == self.rs1 and self.c_rs1_oen) or (EX.reg_rd == self.rs2 and self.c_rs2_oen))

        M1_mem_read = M1.reg_c_dmem_en and M1.reg_c_dmem_rw == M_XRD
        pprev_load_use_hazard = (M1_mem_read and (M1.reg_rd != 0)) and \
                                ((M1.reg_rd == self.rs1 and self.c_rs1_oen) or (M1.reg_rd == self.rs2 and self.c_rs1_oen))

        memory_hazard = self.c_dmem_en and EX.reg_c_dmem_en

        hazard = memory_hazard or prev_load_use_hazard or pprev_load_use_hazard

        # Check misprediction
        pc_sel =            PC_BRJMP if (EX.reg_c_br_type == BR_NE and (not Pipe.EX.alu_out)) or \
                        ...
                        else PC_JALR if EX.reg_c_br_type == BR_JR \
                        else PC_4

        taken = pc_sel != PC_4
        # real target: if not taken -> EX.pp4, if taken: jump target
        self.correct_jump_target = Pipe.EX.pcplus4 if not taken \
                      else Pipe.EX.brjump_target if pc_sel == PC_BRJMP \
                      else Pipe.EX.jump_reg_target if pc_sel == PC_JALR \
                      else WORD(0)

        misprediction = Pipe.EX.pcplus4 != 0 and self.pc != self.correct_jump_target

        if not misprediction:
            self.c_pc_sel = PC_4
        else:
            self.c_pc_sel = PC_MISPREDICT

        # On stall: IF, ID: stall, EX: bubble
        # On c_misprediction: ID, EX: bubble
        self.IF_stall = hazard
        self.ID_stall = hazard
        self.ID_bubble = misprediction
        self.EX_bubble = hazard or misprediction


        # Forwarding from WB is needed
        # Forwarding from M1 is also needed (when M1 stage instruction doesn't use dmem)
        # For alu operand 1
        self.c_fwd_op1 =      FWD_EX if (EX.reg_c_rf_wen and (EX.reg_rd != 0) and self.c_rs1_oen) and \
                                        (EX.reg_rd == self.rs1) \
                         else FWD_M1 if (Pipe.M1.c_rf_wen and (M1.reg_rd != 0) and self.c_rs1_oen) and \
                                        (M1.reg_rd == self.rs1) \
                         else FWD_M2 if (Pipe.M2.c_rf_wen and (M2.reg_rd != 0) and self.c_rs1_oen) and \
                                        (M2.reg_rd == self.rs1) \
                         else FWD_WB if (WB.reg_c_rf_wen and (WB.reg_rd != 0) and self.c_rs1_oen) and \
                                        (WB.reg_rd == self.rs1) \
                         else FWD_NONE

        # For alu operand 2
        self.c_fwd_op2 =      FWD_EX if (EX.reg_c_rf_wen and (EX.reg_rd != 0) and self.c_op2_sel == OP2_RS2) and \
                                        (EX.reg_rd == self.rs2) \
                         else FWD_M1 if (Pipe.M1.c_rf_wen and (M1.reg_rd != 0) and self.c_op2_sel == OP2_RS2) and \
                                        (M1.reg_rd == self.rs2) \
                         else FWD_M2 if (Pipe.M2.c_rf_wen and (M2.reg_rd != 0) and self.c_op2_sel == OP2_RS2) and \
                                        (M2.reg_rd == self.rs2) \
                         else FWD_WB if (WB.reg_c_rf_wen and (WB.reg_rd != 0) and self.c_op2_sel == OP2_RS2) and \
                                        (WB.reg_rd == self.rs2) \
                         else FWD_NONE

        # For rs2 (sw or branch)
        self.c_fwd_rs2 =     FWD_EX if (EX.reg_c_rf_wen and (EX.reg_rd != 0) and self.c_rs2_oen) and \
                                       (EX.reg_rd == self.rs2) \
                        else FWD_M1 if (Pipe.M1.reg_c_rf_wen and (M1.reg_rd != 0) and self.c_rs2_oen) and \
                                       (M1.reg_rd == self.rs2) \
                        else FWD_M2 if (Pipe.M2.reg_c_rf_wen and (M2.reg_rd != 0) and self.c_rs2_oen) and \
                                       (M2.reg_rd == self.rs2) \
                        else FWD_WB if (WB.reg_c_rf_wen and (WB.reg_rd != 0) and self.c_rs2_oen) and \
                                       (WB.reg_rd == self.rs2) \
                        else FWD_NONE
        #-------------------------------------------------------------
        ...
        self.op1_data = self.pc if self.c_op1_sel == OP1_PC \
                        else Pipe.EX.alu_out if self.c_fwd_op1 == FWD_EX \
                        else Pipe.M1.alu_out  if self.c_fwd_op1 == FWD_M1 \
                        else Pipe.M2.wbdata  if self.c_fwd_op1 == FWD_M2 \
                        else Pipe.WB.wbdata  if self.c_fwd_op1 == FWD_WB \
                        else rf_rs1_data

        # Value of this stage
        rs2_or_imm = rf_rs2_data if self.c_op2_sel == OP2_RS2 \
                     else imm_i  if self.c_op2_sel == OP2_IMI \
                     else imm_s  if self.c_op2_sel == OP2_IMS \
                     else imm_b  if self.c_op2_sel == OP2_IMB \
                     else imm_u  if self.c_op2_sel == OP2_IMU \
                     else imm_j  if self.c_op2_sel == OP2_IMJ \
                     else WORD(0)

        self.op2_data = Pipe.EX.alu_out if self.c_fwd_op2 == FWD_EX \
                        else Pipe.M1.alu_out if self.c_fwd_op2 == FWD_M1 \
                        else Pipe.M2.wbdata if self.c_fwd_op2 == FWD_M2 \
                        else Pipe.WB.wbdata if self.c_fwd_op2 == FWD_WB \
                        else rs2_or_imm

        self.rs2_data = Pipe.EX.alu_out if self.c_fwd_rs2 == FWD_EX \
                        else Pipe.M1.alu_out if self.c_fwd_rs2 == FWD_M1 \
                        else Pipe.M2.wbdata if self.c_fwd_rs2 == FWD_M2 \
                        else Pipe.WB.wbdata if self.c_fwd_rs2 == FWD_WB \
                        else rf_rs2_data
...
class EX(Pipe):
    ...
    def compute(self):
        ...
        # BR_J: doesn't use ALU
        self.alu2_data          = self.rs2_data if self.c_br_type in (BR_NE, BR_EQ, BR_GE, BR_GEU, BR_LT, BR_LTU) \
                                  else self.op2_data
        ...
        # For jal/branch
        self.brjump_target = Pipe.cpu.adder_brtarget.op(self.pc, self.op2_data)
...
class M1(Pipe):
    ...
    def compute(self):
        ...
        # DO NOT TOUCH -----------------------------------------------
        # Access dmem using access2(): MM_STAGE1
